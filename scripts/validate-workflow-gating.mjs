...
const requiredValidation = [
  'npm run validate', 'npm run test:rules', 'npx playwright test tests/cloud-first-configured-session.spec.mjs tests/cloud-first-a11y.spec.mjs tests/browser-smoke.spec.mjs tests/single-workspace-board-ux.spec.mjs --grep="cloud-first board|Filters stays bounded|configured signed-out build|board manager|Data recovery route|New board is visible|archived owner board"',
