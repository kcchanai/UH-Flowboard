...
const [html, app, core, main, runtimeBootstrap, contract, localAdapter, firebaseAdapter, cloudAdapter, migrationAdapter, deletionAdapter, lifecycleAdapter, authUI, cloudUI, confirmationUI, inviteUI, membersUI, cloudSync, activityUI, assignmentUI, commentsUI, rules] = await Promise.all([
  ...
  'src/auth-ui.js', 'src/cloud-workspace-ui.js', 'src/confirmation-dialog-ui.js', 'src/invite-ui.js', 'src/members-ui.js', 'src/cloud-sync-controller.js', 'src/activity-ui.js', 'src/assignment-ui.js', 'src/comments-ui.js', 'firestore.rules'
...
for (const copy of ['Authenticated cloud comments are separate from older card-local activity.', 'Comments are current.']) if (html.includes(copy) || commentsUI.includes(copy)) throw new Error(`Static validation failed: retired comments copy remains: ${copy}.`);
for (const token of ['id="cloud-comments-status"', 'aria-live="polite"']) if (!html.includes(token)) throw new Error(`Static validation failed: comments live region is missing: ${token}.`);
for (const token of ['Loading cloud comments...', 'Cloud comments stopped updating.', 'The comment change failed.', "status.textContent=''"]) if (!commentsUI.includes(token)) throw new Error(`Static validation failed: comments operational status path is missing: ${token}.`);
...
if (!confirmationUI.includes('requestConfirmation') || !confirmationUI.includes('trigger?.focus') || !confirmationUI.includes('requestAnimationFrame')) throw new Error('Neutral confirmation module is incomplete.');
if (!cloudUI.includes('u.append(t,d)') || !confirmationUI.includes('dialog.showModal')) throw new Error('Archived board rendering or confirmation behavior is incomplete.');
...
if (!migrationAdapter.includes('importLegacyWorkspace') || !migrationAdapter.includes('exportCloudBackup') || !localAdapter.includes('saveLegacyMigrationReceipt')) throw new Error('Protected legacy maintenance boundary is incomplete.');
