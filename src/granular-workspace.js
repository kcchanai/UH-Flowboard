...
export function granularizeBoard(snapshot, rank) {
  ...
  board.lists.forEach((list, listRank) => {
    ...
    lists.push({id:list.id, rank:listRank, ...listData, lifecycleState:'active'});
    listCards.forEach((card, cardRank) => {
      ...
      cardIds.add(card.id);
      cards.push({id:card.id, listId:list.id, rank:cardRank, ...card, lifecycleState:'active'});
    });
  });
  const {lists: ignored, ...metadata} = board;
  return {board:{id:board.id, rank, ...metadata, lifecycleState:'active'}, lists, cards};
}
