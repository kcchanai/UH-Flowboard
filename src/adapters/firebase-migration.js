...
const userFor=auth=>{if(!auth.currentUser)throw fail('Sign in required.','AUTH_REQUIRED');return auth.currentUser;};
...
async function verifyDocuments(root,documents,marker,operationId){
  for(const group of groups(documents)){
    ...
    snapshots.forEach((snapshot,index)=>{const expected=group[index];if(!snapshot.exists()||snapshot.data()[marker]!==operationId||!same(snapshot.data(),expected.data))throw fail('Verification mismatch.','MIGRATION_VERIFICATION_FAILED');});
  }
}
async function writeDocuments(db,root,documents,marker,operationId,adopt=false){
  for(const group of groups(documents)){
    ...
    group.forEach((item,index)=>{const result=reads[index];if(result.status==='rejected'&&!String(result.reason?.code||'').endsWith('permission-denied'))throw result.reason;const current=result.status==='fulfilled'?result.value:null,prior=current?.data()||{},exists=Boolean(current?.exists()),foreign=exists&&prior[marker]!==operationId;if(foreign&&!(adopt&&prior[marker]===undefined&&same(prior,item.data)))throw fail('Target conflict.','IMPORT_TARGET_CONFLICT');if(exists&&same(prior,item.data)&&prior[marker]===operationId)return;const revision=exists?(same(prior,item.data)?prior.revision||0:(Number.isInteger(prior.revision)?prior.revision+1:0)):0;batch.set(reference(root,item),{...item.data,[marker]:operationId,revision,clientMutationId:operationId,updatedAt:serverTimestamp()});writes+=1;});
...
export async function importLegacyWorkspace(app,auth,{workspaceId,workspace}){
  const user=userFor(auth);if(!id(workspaceId))throw fail('Invalid destination.','INVALID_IMPORT');
  ...
  const initial=await at('receipt',()=>runTransaction(db,async transaction=>{
    ...
    if(current.exists()){const data=current.data();if(data.createdByUid!==user.uid||data.sourceDigest!==sourceDigest||!countsEqual(data.sourceCounts,prepared.counts)||JSON.stringify(data.targetBoardIds)!==JSON.stringify(targetBoardIds))throw fail('Import ID conflict.','IMPORT_IDENTIFIER_CONFLICT');return data.state;}
  ...
  if(initial==='verified')await at('verified-retry',()=>verifyDocuments(root,finalized,'importOperationId',operationId));
    ...
    if(!current.exists()||data.sourceDigest!==sourceDigest||!['importing','verified'].includes(data.state))throw fail('Checkpoint changed.','IMPORT_IDENTIFIER_CONFLICT');
    if(data.state==='verified')return;
    boards.forEach(board=>{if(!board.exists()||board.data().importOperationId!==operationId||board.data().lifecycleState!=='importing')throw fail('Board finalization failed.','MIGRATION_VERIFICATION_FAILED');transaction.update(board.ref,{lifecycleState:'active',revision:(board.data().revision||0)+1,clientMutationId:operationId,updatedAt:serverTimestamp()});});
  ...
  const receipt=await getDocFromServer(record);if(!receipt.exists()||receipt.data().state!=='verified')throw fail('Receipt verification failed.','MIGRATION_VERIFICATION_FAILED');
...
export async function exportCloudBackup(app,auth,workspaceId){
  userFor(auth);const db=getFirestore(app),root=doc(db,'workspaces',workspaceId),scope=await getDocFromServer(root);if(!scope.exists())throw fail('Workspace missing.','WORKSPACE_NOT_FOUND');
...
export async function migrateWorkspaceToGranular(app,auth,workspaceId){
  ...
  const candidate=randomId(),operationId=await runTransaction(db,async transaction=>{const current=await transaction.get(root),data=current.data();if(data.ownerUid!==user.uid||!['ready','migrating'].includes(data.status))throw fail('Migration state changed.','MIGRATION_UNAVAILABLE');if(data.status==='migrating'){if(data.migration?.version!==2||!id(data.migration.operationId))throw fail('Migration cannot resume.','MIGRATION_UNAVAILABLE');return data.migration.operationId;}transaction.update(root,{status:'migrating',migration:{version:2,state:'migrating',operationId:candidate,counts:{boards:0,lists:0,cards:0},startedAt:serverTimestamp()},updatedAt:serverTimestamp()});return candidate;});
  const locked=await getDocFromServer(root);if(!locked.exists()||locked.data().status!=='migrating'||locked.data().migration?.operationId!==operationId)throw fail('Lock verification failed.','MIGRATION_VERIFICATION_FAILED');
  ...
  for(const plan of scan.plans){const documents=migrationDocuments(plan,operationId);await writeDocuments(db,root,documents,'migrationOperationId',operationId,true);const board=await getDocFromServer(plan.source.ref);if(!board.exists()||!board.data().snapshot)continue;await updateDoc(plan.source.ref,{snapshot:deleteField(),granularVersion:1,lastMigrationOperationId:operationId,revision:(board.data().revision??-1)+1,clientMutationId:operationId,updatedAt:serverTimestamp()});const scrubbed=await getDocFromServer(plan.source.ref);if(!scrubbed.exists()||scrubbed.data().snapshot!==undefined)throw fail('Snapshot scrub failed.','MIGRATION_VERIFICATION_FAILED');}
  const finalScan=await scanWorkspace(db,root);if(finalScan.plans.length||!countsEqual(finalScan.counts,scan.counts))throw fail('Count verification failed.','MIGRATION_VERIFICATION_FAILED');
  await updateDoc(root,{status:'ready',migration:{version:2,state:'verified',operationId,counts:finalScan.counts,verifiedAt:serverTimestamp()},updatedAt:serverTimestamp()});const verified=await getDocFromServer(root);if(!verified.exists()||verified.data().status!=='ready'||verified.data().migration?.state!=='verified')throw fail('Upgrade verification failed.','MIGRATION_VERIFICATION_FAILED');
