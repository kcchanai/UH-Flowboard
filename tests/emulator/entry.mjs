...
async function signInFreshHints() {
  ...
  const profile=await getDoc(doc(db,'users',user.uid));if(profile.data()?.personalWorkspaceId)throw new Error('The synthetic hints profile unexpectedly already had a personal pointer.');
  return user;
}
async function signInFreshStageAccount(){const account={email:`stage-${crypto.randomUUID()}@flowboard.test`,password:'Flowboard-stage-123!'},created=await fetch(`${AUTH_EMULATOR}/identitytoolkit.googleapis.com/v1/accounts:signUp?key=${CONFIG.apiKey}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email:account.email,password:account.password,returnSecureToken:true})});if(!created.ok)throw new Error('The Auth Emulator could not create the synthetic stage account.');const {localId}=await created.json(),verified=await fetch(`${AUTH_EMULATOR}/identitytoolkit.googleapis.com/v1/projects/${CONFIG.projectId}/accounts:update?key=${CONFIG.apiKey}`,{method:'POST',headers:{'content-type':'application/json',authorization:'Bearer owner'},body:JSON.stringify({localId,emailVerified:true})});if(!verified.ok)throw new Error('The Auth Emulator could not verify the synthetic stage account.');return{account,uid:localId};}
async function prepareDeniedContent(){const {account,uid}=await signInFreshStageAccount(),user=(await signInWithEmailAndPassword(auth,account.email,account.password)).user,choice=await cloudAdapter.ensurePersonalWorkspace(),boardId=`stage-board-${crypto.randomUUID()}`,listId=`stage-list-${crypto.randomUUID()}`,cardId=`stage-card-${crypto.randomUUID()}`,root=doc(db,'workspaces',choice.workspaceId),nowValue=serverTimestamp();await setDoc(doc(root,'boards',boardId),{id:boardId,title:'Stage probe board',rank:0,archived:false,lifecycleState:'active',revision:0,clientMutationId:`stage-board-${crypto.randomUUID()}`,updatedAt:nowValue});await setDoc(doc(root,'boards',boardId,'lists',listId),{id:listId,title:'Stage probe list',rank:0,lifecycleState:'active',revision:0,clientMutationId:`stage-list-${crypto.randomUUID()}`,updatedAt:nowValue});await setDoc(doc(root,'boards',boardId,'cards',cardId),{id:cardId,listId,title:'Stage probe card',rank:0,assigneeUids:[],labels:[],dueDate:'',checklist:[],archived:false,lifecycleState:'active',revision:0,clientMutationId:`stage-card-${crypto.randomUUID()}`,updatedAt:nowValue});return{uid,workspaceId:choice.workspaceId,boardId,listId,cardId,userUid:user.uid};}
...
const testApi = {
  ...
  async legacyProfileIdentity(){const{account,uid}=await signInFreshStageAccount();await signInWithEmailAndPassword(auth,account.email,account.password);return{uid,emailLower:account.email};},
  async resumeLegacyProfileBootstrap(){allowHintsBootstrap=true;await globalThis.FlowboardApp.setSession(auth.currentUser);},
  async prepareDeniedContent(){return prepareDeniedContent();},
  async existingHintsContext(){try{const user=await signInFreshHints(),choice=await cloudAdapter.ensurePersonalWorkspace();allowHintsBootstrap=true;await globalThis.FlowboardApp.setSession(user);const profile=await getDoc(doc(db,'users',user.uid)),workspace=await getDoc(doc(db,'workspaces',choice.workspaceId));return{state:choice.state,hasPointer:typeof profile.data()?.personalWorkspaceId==='string',hintPreserved:profile.data()?.workspaceIds?.includes('synthetic-legacy-hint')===true,workspacePersonal:workspace.data()?.personal===true,workspaceReady:workspace.data()?.status==='ready',role:choice.entry?.role||''};}catch(error){return{errorCode:error?.code||error?.name||'unknown',errorMessage:String(error?.message||'').slice(0,120)};}},
  ...
  async seedDirectoryPages(){const user=await signInFreshPersonal(),{workspaceId}=await cloudAdapter.ensurePersonalWorkspace();for(let index=0;index<30;index++){const boardId=`paged-board-${String(index).padStart(2,'0')}`;await setDoc(doc(db,'workspaces',workspaceId,'boards',boardId),{id:boardId,title:`Paged board ${String(index+1).padStart(2,'0')}`,rank:100+index,archived:false,lifecycleState:'active',revision:0,clientMutationId:`paged-create-operation-${String(index).padStart(2,'0')}`});}if(seedOnly){allowHintsBootstrap=true;await globalThis.FlowboardApp.setSession(user);}return{workspaceId};},
