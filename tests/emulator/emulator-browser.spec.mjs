...
test('Google identity with mixed-case input bootstraps and creates a real cloud board',async({page})=>{
  ...
  await expect(page.locator('#new-board-button')).toHaveCount(0);
...
test('startup denial stays unavailable until Retry setup creates a verified home and board',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>FlowboardApp.getMode().kind),{timeout:15000}).toBe('cloud');
  await expect(manager.locator('#new-board-button')).toHaveCount(0);await expect(manager.locator('#new-board-form')).toBeVisible();
...
test('historical profile metadata bootstraps through the real adapter and persists a board',async({page})=>{
  await page.goto(`${baseURL}/tests/emulator/index.html?personal=1&legacyProfile=1`);await page.waitForFunction(()=>globalThis.__flowboardEmulatorTest?.ready===true);
  const identity=await page.evaluate(()=>globalThis.__flowboardEmulatorTest.legacyProfileIdentity()),[host,port]=process.env.FIRESTORE_EMULATOR_HOST.split(':'),admin=await initializeTestEnvironment({projectId:'demo-flowboard-browser',firestore:{host,port:Number(port)}}),legacyUpdatedAt=Timestamp.fromMillis(1);
  try{await admin.withSecurityRulesDisabled(async context=>{const batch=writeBatch(context.firestore());batch.set(doc(context.firestore(),'users',identity.uid),{uid:identity.uid,emailLower:identity.emailLower,workspaceIds:['historical-hint'],displayName:'Historical profile name',updatedAt:legacyUpdatedAt});await batch.commit();});await page.evaluate(()=>globalThis.__flowboardEmulatorTest.resumeLegacyProfileBootstrap());await expect.poll(()=>page.evaluate(()=>FlowboardApp.getMode().kind),{timeout:15000}).toBe('cloud');await page.locator('#boards-button').click();const manager=page.locator('#workspace-dialog');await manager.locator('#new-board-title').fill('Historical profile board');await manager.getByRole('button',{name:'Create board'}).click();await expect(manager.locator('#workspace-board-list')).toContainText('Historical profile board');expect(await page.evaluate(()=>globalThis.__flowboardEmulatorTest.personalSummary())).toEqual({signedIn:true,hasPointer:true,workspaceExists:true,role:'owner',boardCount:1});await admin.withSecurityRulesDisabled(async context=>{const profile=await getDoc(doc(context.firestore(),'users',identity.uid));expect(profile.data().displayName).toBe('Historical profile name');expect(profile.data().updatedAt.toMillis()).toBe(legacyUpdatedAt.toMillis());expect(profile.data().workspaceIds).toEqual(expect.arrayContaining(['historical-hint',profile.data().personalWorkspaceId]));});await page.goto(`${baseURL}/tests/emulator/index.html?personal=1`);await page.waitForFunction(()=>globalThis.__flowboardEmulatorTest?.ready===true);await expect.poll(()=>page.evaluate(()=>FlowboardApp.getMode().kind),{timeout:15000}).toBe('cloud');await expect(page.locator('#board-title')).toHaveValue('Historical profile board');}finally{await admin.cleanup();}
});
...
test('same-named boards across same-named scopes stay distinct when a stale scope is missing',async({browser})=>{
  ...
  try{
    ...
    await expect.poll(()=>page.evaluate(()=>globalThis.FlowboardApp.getMode().kind),{timeout:15000}).toBe('cloud');await page.locator('#boards-button').click();const rows=page.locator('#workspace-board-list .workspace-entry').filter({hasText:'Same-named board'});await expect(rows).toHaveCount(2);await expect(rows.filter({hasText:'Source 1 of 2'})).toHaveCount(1);await expect(rows.filter({hasText:'Source 2 of 2'})).toHaveCount(1);await rows.filter({hasText:'Source 2 of 2'}).getByRole('button',{name:/Open Same-named board/}).click();await expect.poll(()=>page.evaluate(expected=>globalThis.FlowboardApp.getMode().id===expected,secondId)).toBe(true);await page.locator('#boards-button').click();const manager=page.locator('#workspace-dialog');await manager.locator('#new-board-title').fill('Personal destination board');await manager.getByRole('button',{name:'Create board'}).click();await expect.poll(()=>page.evaluate(expected=>globalThis.FlowboardApp.getMode().id===expected,personal.workspaceId),{timeout:15000}).toBe(true);await expect(page.locator('#board-title')).toHaveValue('Personal destination board');
...
test('My workspace loads every board page without duplicates and restores focus',async({browser})=>{
  ...
  try{
    await page.goto(`${baseURL}/tests/emulator/index.html?personal=1&seedOnly=1`);await page.waitForFunction(()=>globalThis.__flowboardEmulatorTest?.ready===true);await page.evaluate(()=>globalThis.__flowboardEmulatorTest.seedDirectoryPages());await expect.poll(()=>page.evaluate(()=>globalThis.FlowboardApp.getMode().kind),{timeout:15000}).toBe('cloud');
    await page.locator('#boards-button').click();const manager=page.locator('#workspace-dialog'),paged=manager.locator('#workspace-board-list .workspace-entry').filter({hasText:'Paged board'});await expect(manager.locator('#cloud-workspaces-status')).toHaveText('More boards available.');const firstCount=await paged.count();expect(firstCount).toBeGreaterThan(0);expect(firstCount).toBeLessThan(30);await expect(manager.getByRole('button',{name:/Load more boards/})).toBeVisible();await manager.getByRole('button',{name:/Load more boards/}).click();await expect(paged).toHaveCount(30);await expect(manager.getByRole('button',{name:/Load more boards/})).toHaveCount(0);const labels=await paged.locator('strong').allTextContents();expect(new Set(labels).size).toBe(30);await manager.getByRole('button',{name:'Close boards'}).click();await expect(page.locator('#boards-button')).toBeFocused();
  }finally{await context.close();}
});
...
test('Auth and Firestore Emulator workflow proves discovery, convergence, denial, conflict, revocation, and lifecycle', async ({browser}) => {
  ...
  try {
    ...
    await expect(restoredRow).toContainText('Position');
    await expect(restoredRow).not.toContainText('owner');
