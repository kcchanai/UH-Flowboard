...
test('My workspace loads every board page without duplicates and restores focus',async({browser})=>{
  ...
  try{
    ...
    await page.locator('#boards-button').click();const manager=page.locator('#workspace-dialog'),paged=manager.locator('#workspace-board-list .workspace-entry').filter({hasText:'Paged board'});await expect(manager.locator('#cloud-workspaces-status')).toHaveText('More boards are available. Use Load more for each workspace container.');const firstCount=await paged.count();expect(firstCount).toBeGreaterThan(0);expect(firstCount).toBeLessThan(30);await expect(manager.getByRole('button',{name:/Load more boards from My workspace/})).toBeVisible();await manager.getByRole('button',{name:/Load more boards/}).click();await expect(paged).toHaveCount(30);await expect(manager.getByRole('button',{name:/Load more boards from My workspace/})).toHaveCount(0);const labels=await paged.locator('strong').allTextContents();expect(new Set(labels).size).toBe(30);await manager.getByRole('button',{name:'Close My workspace'}).click();await expect(page.locator('#boards-button')).toBeFocused();
  }finally{await context.close();}
});
